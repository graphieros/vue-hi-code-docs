  ...
  return {
    chart: {
      ...
      highlighter: {
        ...
        crosshairs: {
          show: true,
          stroke: isDarkMode.value ? "#CD9077" : "#1A1A1A",
          strokeDasharray: 4.5,
          strokeWidth: 1.5,
        },
      },
    ...
    line: {
      ...
      dot: {
        useSerieColor: false,
        fill: isDarkMode.value ? "#1A1A1A" : "#FAFAFA",
        strokeWidth: 3,
      },
      tag: {
        followValue: true,
...
onMounted(() => {
  fetch(url.value, {
    ...
    .then((json) => {
      ...
      data.value[0].series = json.downloads
        .map((d) => d.downloads)
        .slice(-120, -1);
...
<template>
  <VueUiXy :dataset="data" :config="config" :key="step">
    <template #menuIcon="{ isOpen }">
      <VueUiIcon
        :name="isOpen ? 'close' : 'knobs'"
        :stroke="isDarkMode ? '#CD9077' : '#1A1A1A'"
      />
    </template>
  </VueUiXy>
</template>
